...
def get_ukraine_city_mappings():
    ...
    for a in soup.select("a[href^='/']"):
        text = a.text.strip()
        match = re.match(r"(.+)\s\(\+38\s?(\d+)\)", text)
        if match:
            city = match.group(1).strip()
            code = match.group(2)
            city_key = city.lower().replace(" ", "_").replace("-", "_").replace("/", "_")
            formatted = f"{city_key}_38_{code}"
...
def scrape_city_results(url, phone_filter="", address_filter=""):
    ...
    for li in li_tags:
        try:
            ...
            phone = re.search(r"\+380\s?\(\d+\)\s?\d+", li.text)
...
def main(city_name=None, area_code=None, search_name="ivan", max_pages=None, phone_filter="", address_filter="", results_limit=None):
    if not max_pages:
        max_pages = 50
    if not results_limit:
        results_limit = 200

    city_map, code_map = get_ukraine_city_mappings()
    all_results = []
    ...
    if city_name:
        ...
        if not formatted_city:
            ...
            return
        print(f"[✓] Using city: {formatted_city}")
        ...
        if not formatted_city:
            print(f"[!] Area code '{area_code}' not found.")
            return
        print(f"[✓] Using area code: {formatted_city}")
        all_results = scrape_all_pages(formatted_city, search_name, max_pages, phone_filter, address_filter, results_limit)

    else:
        print("[ℹ] No city or area code provided. Scraping all Ukrainian cities.")
        for city_key, formatted_city in city_map.items():
            try:
                print(f"[→] Scraping city: {formatted_city}")
                ...
                print(f"[!] Error scraping {formatted_city}: {e}")

    print(f"\n✅ Total results: {len(all_results)}")
    for r in all_results[:results_limit]:
        print(r)

if __name__ == "__main__":
    results = ukraine_search(city_name="Kyiv", search_name="Taras Shevchenko", results_limit=20)
    for r in results:
        print(r)
