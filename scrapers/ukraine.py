...
def get_ukraine_city_mappings():
    res = requests.get(BASE_URL + "/ukraine.html", headers=HEADERS)
    soup = BeautifulSoup(res.text, "html.parser")
    city_map = {}
    code_map = {}
    for a in soup.select("a[href^='/']"):
        text = a.text.strip()
        match = re.match(r"(.+)\s\(\+380\s?\d+\)", text)
        if match:
            city = match.group(1).strip()
            code = match.group(0).split('+380')[-1].strip('() ')
            city_key = city.lower().replace(" ", "_").replace("-", "_").replace("/", "_")
            formatted = f"{city_key}_380_{code}"
...
def ukraine_search(city_name=None, area_code=None, search_name="ivan", max_pages=None, phone_filter="", address_filter="", results_limit=None):
    if not max_pages:
        max_pages = 50
