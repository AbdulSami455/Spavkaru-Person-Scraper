...
def get_moldova_city_mappings():
    res = requests.get(BASE_URL + "/moldova.html", headers=HEADERS)
    soup = BeautifulSoup(res.text, "html.parser")
    city_map = {}
    code_map = {}
    for a in soup.select("a[href^='/']"):
        text = a.text.strip()
        match = re.match(r"(.+)\s\(\+373\s?\d+\)", text)
        if match:
            city = match.group(1).strip()
            code = match.group(0).split('+373')[-1].strip('() ')
            city_key = city.lower().replace(" ", "_").replace("-", "_").replace("/", "_")
            formatted = f"{city_key}_373_{code}"
...
def scrape_city_results(url, phone_filter="", address_filter=""):
    ...
    for li in li_tags:
        try:
            ...
            phone = re.search(r"\+373\s?\d+\s?\d+", li.text)
...
def moldova_search(city_name=None, area_code=None, search_name="ivan", max_pages=None, phone_filter="", address_filter="", results_limit=None):
    if not max_pages:
        max_pages = 50
    if not results_limit:
        results_limit = 200
    city_map, code_map = get_moldova_city_mappings()
    all_results = []
    if city_name:
