...
def get_kazakhstan_city_mappings():
    res = requests.get(BASE_URL + "/kazakhstan.html", headers=HEADERS)
...
def kazakhstan_search(city_name=None, area_code=None, search_name="ivan", max_pages=None, phone_filter="", address_filter="", results_limit=None):
    if not max_pages:
        max_pages = 50
    if not results_limit:
        results_limit = 200
    city_map, code_map = get_kazakhstan_city_mappings()
    all_results = []
    if city_name:
